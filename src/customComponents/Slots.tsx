...
const Slots = () => {
    ...
    const [slots, setSlotsGames] = useState<Game[]>([])
    const { loading, getSlotsGames } = useData()
    ...
    const handleGetGames = async (filter: string) => {
        const result = await getSlotsGames(filter); // This will hold the resolved value\
        console.log(result); // Logs the newGames
        setSlotsGames(result)
    }
    useEffect(() => {
        handleGetGames("")
    }, [])
        ...
        <Card>
            ...
            <CardContent className="space-y-2 ">
                <div className=" grid grid-cols-5 gap-x-3 gap-y-4 sm:grid-cols-3 md:grid-cols-5  ">
                    {slots.map((game, index) => <GameCard game={game} key={index} />)}
                </div>
