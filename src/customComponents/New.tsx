...
const New = () => {
  ...
  const { loading, getNewGames } = useData()
  ...
  const handleGetGames = async (filter: string) => {
    const result = await getNewGames(filter); // This will hold the resolved value\
    ...
    <Card>
      ...
      <CardContent className="space-y-2 ">
        <div className=" grid grid-cols-5 gap-x-3 gap-y-4 sm:grid-cols-3 md:grid-cols-5  ">
          {newGames.map((game, index) => <GameCard game={game} key={index} />)}
        </div>
